...
if (!isset($_GET['aksi'])) {
    ...
    <table>
        ...
        <tbody>
            ...
            if ($siswa && mysqli_num_rows($siswa) > 0) { // Pastikan query berhasil dan mengembalikan hasil
                $no = 1;
                while ($data = mysqli_fetch_array($siswa)) {
                    ...
                    <tr>
                        ...
                        <td>
                            <?php echo isset($data['nis']) ? $data['nis'] : ''; // Periksa apakah NIS ada sebelum mengaksesnya ?>
                        </td>
                        <td>
                            <?php echo isset($data['nama_siswa']) ? $data['nama_siswa'] : ''; // Periksa apakah Nama_Siswa ada sebelum mengaksesnya ?>
                        </td>
                        <td>
                            <?php echo isset($data['jeniss_kelamin']) ? $data['jeniss_kelamin'] : ''; // Periksa apakah Jenis_Kelamin ada sebelum mengaksesnya ?>
                        </td>
                        <td>
                            <?php echo isset($data['alamat']) ? $data['alamat'] : ''; // Periksa apakah Alamat ada sebelum mengaksesnya ?>
                        </td>
                        <td>
                            <?php echo isset($data['foto_mahasiswa']) ? $data['foto_mahasiswa'] : ""; // periksa apakah foto sudah ada ?>
                        <td>
                        <td>
        ...
        if (isset($_POST['simpan'])) {
            ...
            if ($filename != '') {
                if (move_uploaded_file($_FILES['e']['tmp_name'], $uploadfile)) {
                    mysqli_query($koneksi, "INSERT INTO siswa (nis, nama_siswa, jeniss_kelamin, alamat, foto_siswa) 
            ...
            window.location='index.php?page=siswa&aksi=tambah'</script>";
                ...
                mysqli_query($koneksi, "INSERT INTO siswa (nis, nama_siswa, jeniss_kelamin, alamat) 
    ...
    ?>
        <form action='' method="POST" enctype='multipart/form-data'>
            ...
            <label>Jenis Kelamin</label>
            <input type="text" name="c" value="<?php echo $data['jeniss_kelamin']; ?>"><br>
            <label>Alamat</label>
